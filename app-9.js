...
(()=>{
 ...
 const bandWithInstagram=()=>{let html=originalBand();const section=instagramSection();if(!section)return html;const close='</div></section></main>';const i=html.lastIndexOf(close);if(i>=0)return html.slice(0,i)+section+html.slice(i);const mainClose='</main>';const j=html.lastIndexOf(mainClose);return j>=0?html.slice(0,j)+section+html.slice(j):html+section};

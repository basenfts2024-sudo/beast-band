...
(()=>{
 ...
 let siteSocials=socials.filter(s=>String(s[0]||'').toLowerCase()!=='bandcamp');
 ...
 fetch('/api/content?ts='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():null).then(c=>{
  ...
  if(Array.isArray(c?.socials)){siteSocials=c.socials.filter(s=>String(s?.name||'').toLowerCase()!=='bandcamp').filter(s=>String(s?.url||'').trim()).map(s=>[String(s.name||'').trim(),String(s.handle||'').trim(),String(s.url||'').trim()]);}
  render();
 }).catch(()=>{});
})();
